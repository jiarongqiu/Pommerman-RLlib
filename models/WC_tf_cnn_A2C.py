...
class TFCNN(TFModelV2):
    ...
    def init(self):
        ...
        net = _board
        net = tf.keras.layers.Conv2D(64, 3, strides=1, padding="same", activation=tf.nn.relu, kernel_initializer='random_uniform')(net)
        net = tf.keras.layers.BatchNormalization()(net)
        net = tf.keras.layers.Dropout(rate=0.05)(net)
        ...
        net = tf.keras.layers.Dense(512, activation=tf.nn.relu, kernel_initializer='random_uniform')(net)

        net = tf.keras.layers.Dropout(rate=0.01)(net)

        net = tf.keras.layers.Dense(512, activation=tf.nn.relu, kernel_initializer='random_uniform')(net)

        net = tf.keras.layers.Dropout(rate=0.01)(net)

        actor = tf.keras.layers.Dense(128, activation=tf.nn.relu, kernel_initializer='random_uniform')(net)

        value = tf.keras.layers.Dense(128, activation=tf.nn.relu, kernel_initializer='random_uniform')(net)


        action_out = tf.keras.layers.Dense(self.num_outputs, activation=tf.nn.relu)(actor)
        value_out = tf.keras.layers.Dense(1, activation=tf.nn.tanh)(value)
